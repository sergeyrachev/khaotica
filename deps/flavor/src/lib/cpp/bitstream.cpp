...
void Bitstream::Create(const char *filename, Bitstream_t t)
{
    type=t;
    close_fd=1;
    end=0;
    ...
    buf = new unsigned char[BS_BUF_LEN];
...
void Bitstream::Create(int fd, Bitstream_t t)
{
    type=t;
    close_fd=0;
    cur_bit=0;
...
char* const Bitstream::err2msg(Error_t code)
{
    switch (code) {
...
unsigned int Bitstream::nextbits(int n)
{
...
unsigned int Bitstream::snextbits(int n) 
{
...
unsigned int Bitstream::getbits(int n)
{
    register unsigned int x = nextbits(n);
    cur_bit += n;
    tot_bits += n;
...
unsigned int Bitstream::sgetbits(int n)
{
...
// probe a float
float Bitstream::nextfloat(void)
{
    float f;
    *(int *)&f = nextbits(32);
    return f;
...
// probe a double
double Bitstream::nextdouble(void)
{
    double d;
...
// get a double
double Bitstream::getdouble(void)
{
    double d;
    *(((int *)&d)+1) = getbits(32);
    *(int *)&d = getbits(32);
...
int Bitstream::putbits(unsigned int value, int n)
{
...
// put a float
float Bitstream::putfloat(float value)
{
...
// put a double 
double Bitstream::putdouble(double value)
{
...
// returns 'n' bits as unsigned int; does not advance bit pointer
unsigned int Bitstream::little_nextbits(int n)
{
...
// returns 'n' bits as unsigned int with sign extension; does not advance bit pointer (sign extension only if n>1)
unsigned int Bitstream::little_snextbits(int n) 
{
...
// returns 'n' bits as unsigned int; advances bit pointer
unsigned int Bitstream::little_getbits(int n)
{
    register unsigned int x = 0;        // the value we will return
    int bytes = n >> 3;                 // number of bytes to read 
...
// returns 'n' bits as unsigned int with sign extension; advances bit pointer (sign extension only if n>1)
unsigned int Bitstream::little_sgetbits(int n)
{
...
// probe a float
float Bitstream::little_nextfloat(void)
{
    float f;
...
// get a float
float Bitstream::little_getfloat(void)
{
    float f;
...
// probe a double
double Bitstream::little_nextdouble(void)
{
    double d;
    // make sure we have enough data (so that we can go back)
    if (cur_bit + 64 > (buf_len<<3)) fill_buf();
...
// get a double
double Bitstream::little_getdouble(void)
{
    double d;
...
// can only write at least one byte to a file at a time; returns the output value
int Bitstream::little_putbits(unsigned int value, int n)
{
...
// put a float
float Bitstream::little_putfloat(float value)
{
...
// put a double 
double Bitstream::little_putdouble(double value)
{
    little_putbits(*(int *)&value, 32);
 ...
 */
unsigned int Bitstream::next(int n,  int big, int sign, int alen)
{
 ...
 */
unsigned int Bitstream::nextcode(unsigned int code, int n, int alen)
{
    unsigned int s = 0;
...
void Bitstream::flush_buf()
{
    int l = (cur_bit >> 3);     // number of bytes written already
