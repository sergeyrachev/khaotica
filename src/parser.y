...
builtin_function
    ...
    | NEXTCODE '(' BIT_LITERAL ')' {
        $$ = node(flavor::Parser::token::NEXTCODE, leafb(flavor::Parser::token::BIT_LITERAL, $3), NULL);
        $$->type=flavor::Parser::token::INT; $$->ptype = s_lookup("int");
    }
    | NEXTCODE '(' alignment_modifier ',' BIT_LITERAL ')' {
        $$=node(flavor::Parser::token::NEXTCODE, leafb(flavor::Parser::token::BIT_LITERAL, $5), $3);
        $$->type = flavor::Parser::token::INT; $$->ptype = s_lookup("int");
    }
    | NUMBITS '(' ')' { $$ = leaf(flavor::Parser::token::NUMBITS); $$->type = flavor::Parser::token::INT; $$->ptype = s_lookup("int"); }
    ;
...
declaration_list
    : type_info init_declarator {
        ...
        if ($1->usenext && $1->e1->op == flavor::Parser::token::MAP_TYPE && $1->e1->left.sp->escape)
        ...
        if ($1->e1 && $1->e1->op == flavor::Parser::token::BAC_TYPE)
            if ($2 == NULL || $2->e3 == NULL)
        ...
        if ($1->sp->ident == flavor::Parser::token::BIT) $1->type |= M_UNSIGNED;
        ...
        if ($2->e1 != NULL && $2->e1->op == flavor::Parser::token::ARRAY && $2->sp->cxt_level > 0)
            $$ = c_array_unroll($$);
    }
    | declaration_list ',' init_declarator {
        ...
        /* if we have a static array declaration of a non-global variable, unroll it */
        if ($3->e1 != NULL && $3->e1->op == flavor::Parser::token::ARRAY && $3->sp->cxt_level > 0)
...
parsable_type_info
    ...
    | builtin_type_specifier '(' MAP_TYPE ')'             { $$ = lnew_type(M_NONE, $1, leafs(flavor::Parser::token::MAP_TYPE, $3), 0); }
    | user_type_specifier '(' MAP_TYPE ')'                { $$ = lnew_type(M_NONE, $1, leafs(flavor::Parser::token::MAP_TYPE, $3), 0); }
    | builtin_type_specifier '(' conditional_expr ')' '*' { $$ = lnew_type(M_NONE, $1, $3, 1); }
    | builtin_type_specifier '(' MAP_TYPE ')' '*'         { $$ = lnew_type(M_NONE, $1, leafs(flavor::Parser::token::MAP_TYPE, $3), 1); }
    | user_type_specifier '(' MAP_TYPE ')' '*'            { $$ = lnew_type(M_NONE, $1, leafs(flavor::Parser::token::MAP_TYPE, $3), 1); }
    ...
    | declaration_modifiers '(' MAP_TYPE ')' {
        if (!($1 & (M_SHORT | M_LONG))) yyerror("No storage class specified");
        $$ = lnew_type(check_modtype($1, s_lookup("int")), s_lookup("int"), leafs(flavor::Parser::token::MAP_TYPE, $3), 0);
    }
    | declaration_modifiers '(' conditional_expr ')' '*' {
        if (!($1 & (M_SHORT | M_LONG))) yyerror("No storage class specified");
    ...
    | declaration_modifiers '(' MAP_TYPE ')' '*' {
        if (!($1 & (M_SHORT | M_LONG))) yyerror("No storage class specified");
        $$ = lnew_type(check_modtype($1, s_lookup("int")), s_lookup("int"), leafs(flavor::Parser::token::MAP_TYPE, $3), 1);
    }
    ...
    | declaration_modifiers builtin_type_specifier '(' MAP_TYPE ')' {
        $$ = lnew_type(check_modtype($1, $2), $2, leafs(flavor::Parser::token::MAP_TYPE, $4), 0);
    }
    | declaration_modifiers builtin_type_specifier '(' conditional_expr ')' '*' {
        $$ = lnew_type(check_modtype($1, $2), $2, $4, 1);
    }
    | declaration_modifiers builtin_type_specifier '(' MAP_TYPE ')' '*' {
        $$ = lnew_type(check_modtype($1, $2), $2, leafs(flavor::Parser::token::MAP_TYPE, $4), 1);
    }
    | builtin_type_specifier '(' BAC_TYPE ')' {
        $$ = lnew_type(M_NONE, $1, leafs(flavor::Parser::token::BAC_TYPE, $3), 0);
    }
    ;
...
alignment_modifier
    : /* empty */ { $$ = NULL; }
    | ALIGNED { $$ = leafi(flavor::Parser::token::INT_LITERAL, 8); }
...
array
    : '{' array_expr_list '}' {
        $$ = node(flavor::Parser::token::ARRAY, $2, NULL);
        $$->ptype = $2->ptype;
        $$->type = $2->type;
    }
    | STRING_LITERAL {
        $$ = node(flavor::Parser::token::ARRAY, eval_array_string($1), NULL);
        $$->ptype = s_lookup("char");
        $$->type = flavor::Parser::token::CHAR;
    }
    ;
...
declarator
    : identifier { $$ = lnew_sp(flavor::Parser::token::DECL, $1); }
    | declarator '[' assignment_expr ']' {
    ...
    | declarator '[' '[' assignment_expr ']' ']' {
        ...
        $1->dims++;
        $1->dim = (node_t **)buildarray((void **)$1->dim, node(flavor::Parser::token::DBL_BRACE_L, check_array_index($4), NULL));
        $$ = $1;
    }
    ;
...
labeled_statement
    : identifier ':' statement_or_declaration
    {
        ...
        $1->ident = flavor::Parser::token::LABEL;
        $$ = prep($3, lnew_sp(flavor::Parser::token::LABEL, $1));
    }
    | CASE conditional_expr ':' statement_or_declaration
    {
        if ($2->type > flavor::Parser::token::INT) yyerror("The 'case' expression must be of integral type");
        if (!is_literal($2->op) || ($2->op == flavor::Parser::token::VARIABLE && !($2->left.sp->modifiers & M_CONST))) yyerror("The 'case' expression must be a constant");
        $$ = lnews1($1, $2, $4);
    }
...
expression_statement
    : ';'                   { $$ = lnew0(flavor::Parser::token::EXPR); }
    | assignment_expr ';'   { $$ = lnew1(flavor::Parser::token::EXPR, $1); }
    ;
...
selection_statement
    ...
    {
        if ($3->type>flavor::Parser::token::INT) yyerror("'switch' expression must be of integral type");
        $$ = lnews1($1, $3, $5);
    }
    ;
...
class_definition
    : complex_class_declarator class_body
    {
        ...
        $$ = lnews_sp(flavor::Parser::token::CLASS, $1, $2);
    }
    | complex_class_declarator ';'  /* forward declaration */
    {
        ...
        s_remcontext();         /* exit class scope (will not be deleted) */
        s_set_cur_class(NULL);  /* exiting class decl */
        $$ = lnews_sp(flavor::Parser::token::CLASS, $1, NULL);
    }
    ;
...
class_declarator
    : CLASS identifier
    {
        ...
        $2->cxt = s_newcontext(flavor::Parser::token::CLASS);
        /* yet another context for parameter list, if any */
        s_newcontext(0);
        $2->ident = flavor::Parser::token::CLASS_TYPE;
        $$ = $2;
    }
    | ABSTRACT CLASS identifier
    {
        ...
        s_set_cur_class($3);
        /* new context for class members */
        $3->cxt = s_newcontext(flavor::Parser::token::CLASS);
        /* yet another context for parameter list, if any */
        s_newcontext(0);
        $3->ident = flavor::Parser::token::CLASS_TYPE;
        $3->abstract = 1;
        $$ = $3;
    }
    | CLASS CLASS_TYPE
...
id_declarator
    ...
    {
        $$ = set_var(check_rescope_id($6), $2, M_CONST, check_parse($2, M_CONST, leafs(flavor::Parser::token::MAP_TYPE, $4)), $8, NULL, $1, 0, NULL, 0, 0);
        $$->align = $1;
    }
    | alignment_modifier declaration_modifiers builtin_type_specifier '(' MAP_TYPE ')' identifier '=' conditional_expr
    {
        $$ = set_var(check_rescope_id($7), $3, check_modtype(($2|M_CONST), $3), check_parse($3, ($2|M_CONST), leafs(flavor::Parser::token::MAP_TYPE, $5)), $9, NULL, $1, 0, NULL, 0, 0);
        $$->align = $1;
    }
    | alignment_modifier declaration_modifiers '(' MAP_TYPE ')' identifier '=' conditional_expr
    {
        if (!($2 & (M_SHORT | M_LONG))) yyerror("No storage class specified for ID variable '%s'", $6->name);
        $$ = set_var(check_rescope_id($6), s_lookup("int"), check_modtype(($2|M_CONST), s_lookup("int")), check_parse(s_lookup("int"), ($2|M_CONST), leafs(flavor::Parser::token::MAP_TYPE, $4)), $8, NULL, $1, 0, NULL, 0, 0);
        $$->align = $1;
    }
    ;
...
id_range_declarator
    ...
    {
        $$ = set_var(check_rescope_id($6), $2, M_CONST, check_parse($2, M_NONE, leafs(flavor::Parser::token::MAP_TYPE, $4)), idrange($8, $10), NULL, $1, 0, NULL, 0, 1);
        $$->align = $1;
    }
    ...
    {
        $$ = set_var(check_rescope_id($7), $3, check_modtype($2, $3), check_parse($3, ($2|M_CONST), leafs(flavor::Parser::token::MAP_TYPE, $5)), idrange($9, $11), NULL, $1, 0, NULL, 0, 1);
        $$->align = $1;
    }
    | alignment_modifier declaration_modifiers '(' MAP_TYPE ')' identifier '=' conditional_expr RANGE conditional_expr
    {
        if (!($2 & (M_SHORT | M_LONG))) yyerror("No storage class specified for ID variable '%s'", $6->name);
        $$ = set_var(check_rescope_id($6), s_lookup("int"), check_modtype($2, s_lookup("int")), check_parse(s_lookup("int"), ($2|M_CONST), leafs(flavor::Parser::token::MAP_TYPE, $4)), idrange($8, $10), NULL, $1, 0, NULL, 0, 1);
        $$->align = $1;
    }
    ;
...
pragma
    : PRAGMA                { $$ = lnew0(flavor::Parser::token::PRAGMA); }
    | PRAGMA pragma_list    { $$ = lnews0(flavor::Parser::token::PRAGMA, $2); }
    ;
...
pragma_item
    : PRAGMA_ARRAY '=' INT_LITERAL {
        ...
        $$ = lnew1($1, leafi(flavor::Parser::token::INT_LITERAL, max_array_size));
    }
    | PRAGMA_GET		        { $$ = lnew0($1); }
    ...
    | PRAGMA_BITSTREAM '=' STRING_LITERAL   { $$ = lnew1($1, leafstr(flavor::Parser::token::STRING_LITERAL, $3)); }
    | PRAGMA_PREFIX '=' STRING_LITERAL      { $$ = lnew1($1, leafstr(flavor::Parser::token::STRING_LITERAL, $3)); }
    | PRAGMA_ERROR_FUNC '=' STRING_LITERAL  { $$ = lnew1($1, leafstr(flavor::Parser::token::STRING_LITERAL, $3)); }
    | PRAGMA_TRACE_FUNC '=' STRING_LITERAL  { $$ = lnew1($1, leafstr(flavor::Parser::token::STRING_LITERAL, $3)); }
    ;
...
import_file
    : IMPORT STRING_LITERAL {
        $$ = lnew1(flavor::Parser::token::IMPORT, leafstr(flavor::Parser::token::STRING_LITERAL, $2));
        if (!include($2, 1)) {
...
include_file
    : INCLUDE STRING_LITERAL {
        $$ = lnew1($1, leafstr(flavor::Parser::token::STRING_LITERAL, gethname($2)));
        if (!include($2, 0)) {
            /* loop would occur; don't issue include statement */
...
map_definition
    : map_declarator map_body {
        ...
        $$ = lnews_sp(flavor::Parser::token::MAP, $1, $2);
    }
    ;

map_declarator
    : MAP identifier '(' type_specifier ')' {
        $2->ident = flavor::Parser::token::MAP_TYPE;
        $2->modifiers = M_NONE;
        $2->ptype = $4;
        if ($4->ident == flavor::Parser::token::CLASS_TYPE) check_map_class($4);
...
map_code
    : BIT_LITERAL { $$ = lnew1(flavor::Parser::token::BIT_LITERAL, leafb(flavor::Parser::token::BIT_LITERAL, $1)); }
    ;
...
map_entry_item_list
    : conditional_expr                         { $$ = lnew1(flavor::Parser::token::EXPR, $1); }
    | map_entry_item_list ',' conditional_expr { $$ = app($1, lnew1(flavor::Parser::token::EXPR, $3)); }
    ;

map_entry_item_escape
    : builtin_type_specifier '(' conditional_expr ')' {
        $$ = lnew3(flavor::Parser::token::ESC_FTYPE, leafi(flavor::Parser::token::EXPR, M_NONE), leafbs(flavor::Parser::token::EXPR, $1), $3);
    }
    | declaration_modifiers builtin_type_specifier '(' conditional_expr ')' {
        $$ = lnew3(flavor::Parser::token::ESC_FTYPE, leafi(flavor::Parser::token::EXPR, check_modtype($1, $2)), leafbs(flavor::Parser::token::EXPR, $2), $4);
    }

    /* The following elements will be used if and when we allow map cascading.

    | CLASS_TYPE '(' MAP_TYPE ')' {
        $$ = lnew3(flavor::Parser::token::ESC_MAP, leafi(flavor::Parser::token::EXPR, M_NONE), leafs(flavor::Parser::token::CLASS_TYPE, $1), leafs(flavor::Parser::token::MAP_TYPE, $3));
    }
    | builtin_type_specifier '(' MAP_TYPE ')' {
        $$ = lnew3(flavor::Parser::token::ESC_MAP, leafi(flavor::Parser::token::EXPR, M_NONE), leafs(flavor::Parser::token::EXPR, $1), leafs(flavor::Parser::token::MAP_TYPE, $3));
    }
    | declaration_modifiers builtin_type_specifier '(' MAP_TYPE ')' {
        $$ = lnew3(flavor::Parser::token::ESC_MAP, leafi(flavor::Parser::token::EXPR, check_modtype($1, $2)), leafs(flavor::Parser::token::EXPR, $2), leafs(flavor::Parser::token::MAP_TYPE, $4));
    }
    */
    ;
...
map_entry_item
    : conditional_expr { $$ = check_map_simple_value(s_get_cur_class(), lnew1(flavor::Parser::token::EXPR, $1)); }
...
bac_definition
    : bac_declarator bac_body
    {
        ...
        $$ = lnews_sp(flavor::Parser::token::BAC, $1, $2);
    }
    ;
...
bac_declarator
    : BAC identifier
    {
        $2->ident = flavor::Parser::token::BAC_TYPE;
        $2->modifiers = M_NONE;
        $$ = s_set_cur_class($2);
    }
    ;
...
bac_keyword
    : STRING_LITERAL { $$ = lnew1(flavor::Parser::token::INT_LITERAL, leafi(flavor::Parser::token::INT_LITERAL, check_bac_param($1))); }
    ;

bac_entry_item
    : conditional_expr              { $$ = lnew1(flavor::Parser::token::EXPR, $1); }
    | STRING_LITERAL                { $$ = lnew1(flavor::Parser::token::STRING_LITERAL, leafstr(flavor::Parser::token::STRING_LITERAL, $1)); }
...
bac_entry_item_list
    : conditional_expr                          { $$ = lnew1(flavor::Parser::token::EXPR, $1); }
    | STRING_LITERAL                            { $$ = lnew1(flavor::Parser::token::STRING_LITERAL, leafstr(flavor::Parser::token::STRING_LITERAL, $1)); }
    | bac_entry_item_list ',' conditional_expr  { $$ = app($1, lnew1(flavor::Parser::token::EXPR, $3)); }
    | bac_entry_item_list ',' STRING_LITERAL    { $$ = app($1, lnew1(flavor::Parser::token::STRING_LITERAL, leafstr(flavor::Parser::token::STRING_LITERAL, $3))); }
    ;

bac_statement
    : BAC_TYPE '(' INT_LITERAL ')' ';' { $$ = lnew2(flavor::Parser::token::BAC_TYPE, leafs(flavor::Parser::token::BAC_TYPE, $1), leafi(flavor::Parser::token::INT_LITERAL, $3)); }
    ;
