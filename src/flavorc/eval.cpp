...
node_t *eval_ident(symbol_t *ident)
{
    ...
    if (!(ident->modifiers & M_CONST) || ident->dims > 0) return leafs(flavor::Parser::token::VARIABLE, ident);
    ...
    if (!is_literal(ident->init->op)) return leafs(flavor::Parser::token::VARIABLE, ident);
    ...
    np->eval = leafs(flavor::Parser::token::VARIABLE, ident);
...
node_t *eval_incdec(int op, node_t *expr)
{
    ...
    if (expr->ptype->ident == flavor::Parser::token::CLASS_TYPE) {
...
node_t *eval_scoped(node_t *left, symbol_t *right)
{
    ...
    if (left->ptype == NULL || left->ptype->ident != flavor::Parser::token::CLASS_TYPE) {
        ...
        if (right->ptype == NULL) {
            right->ident = flavor::Parser::token::INT;
    ...
    n = node('.', left, leafs(flavor::Parser::token::VARIABLE, right));
...
node_t *eval_unary(int op, node_t *expr)
{
    ...
    if (expr->type == flavor::Parser::token::CLASS_TYPE) {
        yyerror("Operator '%s' cannot be applied to classes", s_op2str(op));
        goto eval_unary_end;
    }
    if (op == flavor::Parser::token::INC || op == flavor::Parser::token::DEC) {
        /* there must be a valid lvalue in the expression */
        if (!check_unary(expr))
            yyerror("Operator '%s' can only be applied on a non-constant, non-parsable variable", s_op2str(op));
        goto eval_unary_end;
    ...
    if (expr->type > flavor::Parser::token::INT && !(op == '+' || op == '-') ) {
    ...
    if (expr->op == flavor::Parser::token::VARIABLE) {
    ...
    if (is_literal(expr->op)) {
        ...
        if (expr->op == flavor::Parser::token::BIT_LITERAL) expr = n_bit2int(node_dup(expr));
        ...
        case '-':
            if (expr->type>flavor::Parser::token::INT) expr->left.dval = -expr->left.dval;
...
node_t *eval_mult(int op, node_t *left, node_t *right)
{
    ...
    if (op == '%' && (left->type > flavor::Parser::token::INT || right->type > flavor::Parser::token::INT)) {
        yyerror("Operator '%s' cannot be applied to floats or doubles", s_op2str(op));
        goto eval_mult_end;
    }
    /* no reduction possible on variables, unless they are const */
    if (left->op == flavor::Parser::token::VARIABLE) {
    ...
    /* same for right side */
    if (right->op == flavor::Parser::token::VARIABLE) {
    ...
    if (is_literal(left->op) && is_literal(right->op)) {
        ...
        if (left->op == flavor::Parser::token::BIT_LITERAL) left = n_bit2int(node_dup(left));
        if (right->op == flavor::Parser::token::BIT_LITERAL) right = n_bit2int(node_dup(right));
        
        switch (op) {
            
        case '*':
            /* int/int */
            if (left->type <= flavor::Parser::token::INT && right->type <= flavor::Parser::token::INT) {
                left->left.ival *= right->left.ival;
                return left;
            }
            /* int/double */
            else if (left->type <= flavor::Parser::token::INT && right->type > flavor::Parser::token::INT) {
...
node_t *eval_add(int op, node_t *left, node_t *right)
{
    ...
    if (is_literal(left->op) && is_literal(right->op)) {
        /* if bitstrings, convert to int */
        if (left->op == flavor::Parser::token::BIT_LITERAL)
            left = n_bit2int(node_dup(left));
        if (right->op == flavor::Parser::token::BIT_LITERAL)
...
node_t *eval_shift(int op, node_t *left, node_t *right)
{
    ...
    if (is_literal(left->op) && is_literal(right->op)) {
        ...
        case flavor::Parser::token::LS:
            left->left.ival <<= right->left.ival;
            return left;
            
        case flavor::Parser::token::RS:
            left->left.ival >>= right->left.ival;
            return left;
    ...
    if (is_literal(left->op) && is_literal(right->op)) {
        ...
        if (left->type <= flavor::Parser::token::INT && right->type <= flavor::Parser::token::INT) {
            ...
            case '>':
                left->left.ival = (left->left.ival > right->left.ival);
                break;
            case flavor::Parser::token::LE:
                left->left.ival = (left->left.ival <= right->left.ival);
                break;
            case flavor::Parser::token::GE:
                left->left.ival = (left->left.ival <= right->left.ival);
                break;
            case flavor::Parser::token::EQ:
                left->left.ival = (left->left.ival == right->left.ival);
                break;
            case flavor::Parser::token::NE:
                left->left.ival = (left->left.ival != right->left.ival);
                break;
            }
            return left;
        ...
        /* double/double */
            ...
            left->type = flavor::Parser::token::INT;
...
node_t *eval_logic(int op, node_t *left, node_t *right)
{
    ...
    if ((left->type > flavor::Parser::token::INT || right->type > flavor::Parser::token::INT) && !(op == flavor::Parser::token::AND || op == flavor::Parser::token::OR))  {
        yyerror("Operator '%s' cannot be applied to floats or doubles", s_op2str(op));
        goto eval_logic_end;
    }
    ...
    if (is_literal(left->op) && is_literal(right->op)) {
        ...
        case '|':
            left->left.ival = (left->left.ival | right->left.ival);
            break;
        case flavor::Parser::token::AND:
            left->left.ival = (left->left.ival && right->left.ival);
            break;
        case flavor::Parser::token::OR:
            left->left.ival = (left->left.ival || right->left.ival);
            break;
        }
        return left;
...
node_t *eval_cond(int op, node_t *cond, node_t *left, node_t *right)
{
    ...
    /* not allowed on classes */
    if (cond->type == flavor::Parser::token::CLASS_TYPE || left->type == flavor::Parser::token::CLASS_TYPE || right->type == flavor::Parser::token::CLASS_TYPE) {
        yyerror("Operator '%s' cannot be applied to classes", s_op2str(op));
        goto eval_cond_end;
    }

    /* no reduction possible on variables, unless they are const */
    if (cond->op == flavor::Parser::token::VARIABLE) {
...
node_t *eval_subarray(node_t *left, node_t *right)
{
    ...
    if (right->type > flavor::Parser::token::INT) {
        if (right->ptype == NULL)
...
node_t *eval_array(node_t *expr)
{
    ...
    if (sp->init->op != flavor::Parser::token::ARRAY) if (is_literal(sp->init->op)) ret = sp->init;
    else {
        ...
        while (n->op == '[') {
            ...
            if (n->right.np->op != flavor::Parser::token::INT_LITERAL)
        ...
        for (i=0; i<sp->dims; i++) {
            ...
            if (n->left.np->op != flavor::Parser::token::INT_LITERAL) goto eval_array_end;
...
node_t *eval_lengthof(node_t *args)
{
    ...
    if (!check_lengthof(pa)) {
        /* create a 0 value */
        ret = leafi(flavor::Parser::token::INT_LITERAL, 0);
        return ret;
    }
    ...
    if (sp->ptype->ident == flavor::Parser::token::CLASS_TYPE) {
        ...
        ret = node('.', pa, leafs(flavor::Parser::token::VARIABLE, sp->plengthof));
...
node_t *eval_isidof(symbol_t *sp, node_t *args)
{
    ...
    if (sp->ident != flavor::Parser::token::CLASS_TYPE) fatal("Internal error EV%d: Expected class type", __LINE__);
    ...
    /* create expression */
    ret = node(flavor::Parser::token::ISIDOF, leafs(flavor::Parser::token::CLASS_TYPE, sp), args);
    ret->type = flavor::Parser::token::INT;
...
node_t *eval_skipbits(node_t *args)
{
    ...
    if (args->type > flavor::Parser::token::INT) {
        yyerror("The skipbits function cannot take a float or double");
        goto eval_skipbits_end;
    }
    
    /* if a bitstring, convert to int */
    if (args->op == flavor::Parser::token::BIT_LITERAL) args = n_bit2int(node_dup(args));
        
eval_skipbits_end:
    ret = node(flavor::Parser::token::SKIPBITS, args, NULL);
    ret->type = flavor::Parser::token::INT;
    ret->ptype = s_lookup("int");
...
node_t *eval_nextbits(node_t *n, unsigned int mod_val, node_t* align_expr)
{
    ...
    if (n->type > flavor::Parser::token::INT) {
        yyerror("The nextbits function cannot take a float or double");
        goto eval_nextbits_end;
    }
    
    /* if bitstrings, convert to int */
    if (n->op == flavor::Parser::token::BIT_LITERAL) n = n_bit2int(node_dup(n));
        
eval_nextbits_end:
    if (mod_val & M_LITTLE) {
        if (mod_val & M_SIGNED) type = flavor::Parser::token::LITTLE_SNEXTBITS;
        else type = flavor::Parser::token::LITTLE_NEXTBITS;
    }
    else {
        if (mod_val & M_SIGNED) type = flavor::Parser::token::SNEXTBITS;
        else type = flavor::Parser::token::NEXTBITS;
    }
...
node_t* eval_array_string(char* str)
{
    ...
    p = str;
    while (*p) {
        if (np == NULL) np = leafi(flavor::Parser::token::INT_LITERAL, *p);
        else np = node(',', np, leafi(flavor::Parser::token::INT_LITERAL, *p));
        p++;
    }
