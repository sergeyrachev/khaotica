...
void c_var_decl(symbol_t *sp, int dims, node_t **dim)
{
    ...
    if (sp->cxt_level > 0) {
        ...
        if (cpp()) {
            ...
            if (sp->ptype->ident == flavor::Parser::token::CLASS_TYPE && cpp())
                c_out("*");
        }
        else if (java()) {
    ...
    if (sp->ptype->ident != flavor::Parser::token::CLASS_TYPE && sp->lengthof) {
        c_outi("int ");
...
void c_var_decl_inclass(symbol_t *sp, node_t *init, node_t *parse, int dims, node_t **dim)
{
    ...
    if (sp->parsable) {
        ...
        if ((sp->lengthof && sp->ptype->ident != flavor::Parser::token::CLASS_TYPE) || (init != NULL && sp->ptype->ident <= flavor::Parser::token::INT))
         ...
         */
        if ((sp->ptype->ident != flavor::Parser::token::CLASS_TYPE && (gen_trace || sp->lengthof)) ||
            (sp->ptype->ident == flavor::Parser::token::CLASS_TYPE && sp->id != NULL && !c_is_pure_abstract(sp->ptype)))
            sp->owner->get_needs_parse_var = 1;
    }
    
    /* if parsable, we are tracing, and uses map, then get needs code var */
    if (gen_trace && sp->parsable && parse != NULL && parse->op == flavor::Parser::token::MAP_TYPE) {
        if (sp->owner == NULL)
            fatal("Internal error CV%d: Expected class member variable", __LINE__);
     ...
     */
    if (sp->owner != NULL) 
        if (init != NULL || sp->parsable || sp->ptype->ident == flavor::Parser::token::CLASS_TYPE)
...
static void c_var_decl_get_parse_simple(symbol_t *sp, node_t *ip, node_t *pp, int dims, node_t **dim, int usenext)
{
    ...
    if (java())
        if (sp->ptype->ident != flavor::Parser::token::BIT && sp->ptype->ident != flavor::Parser::token::INT)
            c_out("(%s) ", sp->ptype->name);

    c_out("%sbs.%s%s%s", prefix, 
	                     (sp->modifiers & M_LITTLE ? "little_" : ""),
	                     (sp->modifiers & M_UNSIGNED || sp->ptype->ident > flavor::Parser::token::INT  ? "" : "s"),
                         (usenext ? "next" : "get"));
    ...
    if (sp->ptype->ident <= flavor::Parser::token::INT) {
        ...
        c_out("float(");
    else if (sp->ptype->ident == flavor::Parser::token::FLOAT && (sp->modifiers & M_LONG))
        c_out("ldouble(");
    else 
        c_out("double(");
    ...
    if (ip != NULL) {
        ...
        if (sp->range) {
            ...
            if (dims > 0) 
                c_array_proper(sp, dims);
            c_out(" != ");
            if (sp->ptype->ident == flavor::Parser::token::FLOAT)
                c_out("(float)");
            c_expression(ip, 0);
            c_out(") {\n");
            c_identlevel++;
...
static void c_var_decl_get_parse_map(symbol_t *sp, node_t *ip, symbol_t *map, int dims, node_t **dim, int usenext)
{
	...
	if (java()) {
		...
		if (sp->ptype->ident <= flavor::Parser::token::INT)
            ...
            c_out("_dout");
		else if (sp->ptype->ident == flavor::Parser::token::CLASS_TYPE)
...
static void c_var_decl_get_parse(symbol_t *sp, node_t *init, node_t *parse, int dims, node_t **dim, int usenext)
{
    ...
    if (is_literal(pp->op)) {
        if (pp->op != flavor::Parser::token::INT_LITERAL)
    ...
    if (pp->op == flavor::Parser::token::MAP_TYPE)
                    ...
                    int         usenext)    /* if > 0 --> lookahead parsing */ 
{
    ...
    if (sp->parsable && parse && parse->op == flavor::Parser::token::BAC_TYPE) {
...
static void c_var_decl_put_parse(symbol_t *sp, node_t *init, node_t *parse, int dims, node_t **dim, int usenext)
{
    ...
    if (pp->op != flavor::Parser::token::MAP_TYPE && (sp->lengthof || ip != NULL)) {
         ...
         */
        if (sp->lengthof || sp->ptype->ident <= flavor::Parser::token::INT) {
            c_outi("%sparse = ", prefix);
            c_expression(pp, 0);
            c_out(";\n");
        }
