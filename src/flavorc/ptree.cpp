...
list_t *app_map(list_t *list, list_t *item)
{
    ...
    /* sanity check */
    if (item == NULL || item->e1 == NULL || item->e1->op != flavor::Parser::token::BIT_LITERAL || item->e1->left.bit == NULL)
...
void destroy(list_t *list)
{
    ...
    while (cur != NULL) {
        next = cur->next;
        if (cur->type == flavor::Parser::token::DECL || cur->type == flavor::Parser::token::LABEL) continue;
...
{
    if (np == NULL) return;
    if (np->op == flavor::Parser::token::BIT_LITERAL) {
        free(np->left.bit);
        np->left.bit = NULL;
    }
    if (np->op == flavor::Parser::token::STRING_LITERAL) {
        free(np->left.string);
        np->left.string = NULL;
    }
...
node_t *leafi(int type, int i)
{
    ...
    np->op = type;
    np->type = flavor::Parser::token::INT;
    np->ptype = s_lookup("int");
    np->leaf = 1;
    np->paren = 0;
    np->eval = NULL;
...
node_t *leafd(int type, double d)
{
    ...
    np->op = type;
    np->type = flavor::Parser::token::DOUBLE;
...
node_t *leafb(int type, bit_t *b)
{
    ...
    np->op = type;
    np->type = flavor::Parser::token::BIT;
...
symbol_t *set_lengthof(symbol_t *var)
{
    ...
    if (var->ptype->ident == flavor::Parser::token::CLASS_TYPE)
        sp = s_create("%slengthof", flavor::Parser::token::VARIABLE);    /* a class */
    else {
        /* not a class */
        /* allocate memory for '%s<name>_lengthof\0' */
        str = (char *)getmem(strlen(var->name)+12);
        sprintf(str, "%%s%s_lengthof", var->name);
        sp = s_create(str, flavor::Parser::token::VARIABLE);
                  ...
                  int           range)
{
    ...
    if (var->ptype != NULL) {
        ...
        if (var->owner != s_get_cur_class())
            return set_var(s_add(var->name, flavor::Parser::token::VARIABLE), type, mod, parse, init, param, align, dims, dim, formal, range);
        ...
        if (type->ident == flavor::Parser::token::CLASS_TYPE && init != NULL) {
            yyerror("Class '%s' cannot be initialized (use parameter types)", var->name);
            return var;
        }
        /* check if params are needed/match */
        if (type->ident == flavor::Parser::token::CLASS_TYPE) {
            if (!formal) {
        ...
        else if (param != NULL && parse->op != flavor::Parser::token::BAC_TYPE)
        ...
        if (init != NULL && init->op == flavor::Parser::token::ARRAY) {
            if (!check_array_init(var, dim, dims, init)) {
                /* rewrite 'init' as a single 0 */
                init->op = flavor::Parser::token::INT_LITERAL;
                init->left.ival = 0;
            }
        }
        ...
        if (init != NULL && is_literal(parse->op))
            ...
            else if (init->op == flavor::Parser::token::ARRAY)
    ...
    if (type->ident == flavor::Parser::token::CLASS_TYPE && type->forward == 1)
    ...
    if (type->ident != flavor::Parser::token::CLASS_TYPE && (mod & M_CONST) && init == NULL)
    ...
    if (init != NULL) {
        ...
        if (init->ptype != NULL && !check_type_compat(var->ptype, init->ptype))
            yyerror("Initial value for '%s' not compatible with type declaration", var->name);
        /* if it is an array, check that the dimensions match (if not, init will be set to 0) */
        if (init->op == flavor::Parser::token::ARRAY)
            if (!check_array_init(var, dim, dims, init)) {
                /* rewrite 'init' as a single 0 */
                init->op = flavor::Parser::token::INT_LITERAL;
                init->left.ival = 0;
        ...
        if (parse != NULL && is_literal(parse->op))
            ...
            else if (init->op == flavor::Parser::token::ARRAY) check_fit_array(var, init, parse);
...
node_t *idrange(node_t *node1, node_t *node2)
{
    ...
    if ( (node1->op == flavor::Parser::token::INT_LITERAL && node1->left.ival > node2->left.ival) ||
         (node1->op == flavor::Parser::token::DOUBLE_LITERAL && node1->left.dval > node2->left.dval) ||
         (node1->op == flavor::Parser::token::BIT_LITERAL && node1->left.bit->val > node2->left.bit->val) ) {
