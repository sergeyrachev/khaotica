...
static symbol_t *get_lvalue(node_t *exp)
{
     ...
     */
    if (exp->op == VARIABLE) return exp->left.sp;
...
int check_lengthof(node_t *args)
{
	...
	if (sp->ptype->ident == CLASS_TYPE || sp->parsable) {
		/* set lengthof flag in this variable; if a class, set in class as well */
		sp->lengthof = 1;
        if (sp->ptype->ident == CLASS_TYPE) sp->ptype->lengthof = 1;
		return 1;
	}
	else {
		/* invalid argument */
...
int check_isidof(symbol_t *c, node_t *args)
{
    ...
    if (args->op == NEXTBITS || args->op == SNEXTBITS || args->op == LITTLE_NEXTBITS || args->op == LITTLE_SNEXTBITS)
        return 1;

    /* find variable, and make sure the type is compatible with the id */
	sp = get_lvalue(args);
    if (sp == NULL) return 0;
    if (sp->ptype->ident == CLASS_TYPE || !check_type_compat(c->id->ptype, sp->ptype)) {
...
{
    if (type1->ident == CLASS_TYPE) return type1;
    else if (type2->ident == CLASS_TYPE) return type2;
...
int check_type_compat(symbol_t *type1, symbol_t *type2)
{
    ...
    if (type1->ident < type2->ident) {
        ...
        if (type2->ident == CLASS_TYPE || (type1->ident <= INT && type2->ident > INT)) return 0;
        else return 1;
    }
    return 1;
}
...
unsigned int check_modtype(unsigned int mod, symbol_t *type)
{
    ...
    if (type->ident != INT && type->ident != CHAR) {
    ...
    if (type->ident != INT && type->ident != CHAR && ((m & M_SHORT) || (m & M_LONG))) {
        if (type->ident != DOUBLE || !(m & M_LONG)) {
    ...
    if (type->ident == BIT) m |= M_UNSIGNED;
...
node_t *check_unassigned(node_t *exp, node_t *root)
{
    ...
    else if (exp->op == ISIDOF) check_unassigned(exp->right.np, root);
...
node_t* check_assign(int op, node_t* left, node_t* right)
{
    ...
    if (left->type>INT) {
        if (op == MOD_ASSIGN || op == LS_ASSIGN  || op == RS_ASSIGN ||
...
static void check_actual_param_dims(symbol_t *sp, node_t *p)
{
    ...
    if (sp->dims > 0)
        if (p->op != VARIABLE || p->left.sp->dims != sp->dims)
...
int check_fit_array(symbol_t *var, node_t *init, node_t *parse)
{
    ...
    if (init->op != ARRAY || init->left.np == NULL)
...
int check_fit(symbol_t* var, node_t* init, node_t* parse)
{
    ...
    if (init->type>INT || parse->type>INT)
        /* TBD: check fit for doubles/floats */
        return 1;
    
    /* figure out parse size */
    if (parse->op == BIT_LITERAL) parse_size = parse->left.bit->val;
    ...
    if (init->op == BIT_LITERAL) val=init->left.bit->val;
...
static int check_array_init_nelem(int *dim_values, int dims, node_t *init, int level)
{
    ...
    while (p != NULL) {
        if (p->op == ',') {
            if (p->right.np == NULL)
                fatal("Internal error CH%d: No subexpression in array initializer", __LINE__);
            if (p->right.np->op == ARRAY && !check_array_init_nelem(dim_values, dims, p->right.np, level+1))
                return 0;
        }
        else if (p->op == ARRAY) {
...
static int check_indices(symbol_t *sp, node_t *expr)
{
    ...
    for (i=sp->dims-1; i>=0; i--) {
        ...
        if (dimp->op == DBL_BRACE_L)
            continue;
        if (dimp->left.np->op == INT_LITERAL && n->right.np->op == INT_LITERAL) 
...
int check_array(node_t *expr, symbol_t **spp, int *literal)
{
    ...
    *literal = 1;
    ...
    while (n->op == '[') {
        ...
        if (n->right.np->op != INT_LITERAL) *literal=0;
        n = n->left.np;
    }
...
node_t* check_parse(symbol_t* type, unsigned int mods, node_t* parse)
{
    ...
    if (parse->op == MAP_TYPE) {
        /* if classes, they must be identical */
        if (type->ident == CLASS_TYPE || parse->ptype->ident == CLASS_TYPE) {
    ...
    if (parse->op == BIT_LITERAL) {
        parse = leafi(INT_LITERAL, parse->left.bit->val);
    }
    /* if double or float, parse size must be a constant */
    if (type->ident == FLOAT || type->ident == DOUBLE) {
        if (parse->op != INT_LITERAL)
    ...
    if (type->ident == CHAR && parse->op == INT_LITERAL) {
    ...
    if (parse->op == INT_LITERAL) {
        if (cpp()) {
            if (parse->left.ival > 32 && parse->left.ival != get_double_size(DOUBLE, 0))
...
node_t* check_double_parse(symbol_t* sp, node_t* parse)
{
    ...
    if (parse->type != INT) return parse;
...
static void check_id_unique_class(node_t *id_init, symbol_t *base)
{
    ...
    if (id_init->op == BIT_LITERAL)
        ivalue = id_init->left.bit->val;
    else if (id_init->op == INT_LITERAL)
        ivalue = id_init->left.ival;
    else if (id_init->op == DOUBLE_LITERAL)
...
symbol_t *check_id(symbol_t *var)
{
    ...
    if (!var->id->range)
         ...
         */
        if (var->id->parse->op == INT_LITERAL) {
    ...
    if (var->id->parse->op == INT_LITERAL) {
        if (base->id->parse->op == INT_LITERAL) {
    ...
    else if (var->id->parse->op == MAP_TYPE) {
        if (base->id->parse->op == INT_LITERAL)
    ...
    if ((var->id->align == NULL && base->id->align != NULL) ||
        ...
        if (var->id->align->op != INT_LITERAL || base->id->align->op != INT_LITERAL)
...
int check_map(symbol_t *map, list_t *codes, list_t *code)
{
    ...
    if (code->e1 == NULL || code->e1->op != BIT_LITERAL || code->e1->left.bit == NULL)
    ...
    while (cur != code && cur != NULL) {
        ...
        if (cur->e1 == NULL || cur->e1->op != BIT_LITERAL || cur->e1->left.bit == NULL)
            fatal("Internal error CH%d: Expected code entry with bit literal", __LINE__);
		...
		if (map->ptype->ident <= INT) {
			if (cur->sub1->e1->left.ival == code->sub1->e1->left.ival && code->sub1->type != ESC_FTYPE) {
				yyerror("Error: Value '%s' conflicts within map '%s'", node2string(code->sub1->e1), map->name);
				return 0;
			}
		}
		/* double or float type */
		else if (map->ptype->ident <= DOUBLE) {
			if (cur->sub1->e1->left.dval == code->sub1->e1->left.dval && code->sub1->type != ESC_FTYPE) {
			    yyerror("Error: Value '%s' conflicts within map '%s'", node2string(code->sub1->e1), map->name);
				return 0;
			}
		}
		/* class type */
		else if (map->ptype->ident != CLASS_TYPE) {
...
int check_map_class(symbol_t *sp)
{
    ...
    while (stmts != NULL) {
        ...
        case DECL:
            ...
            if (stmts->sp->ptype->ident == CLASS_TYPE) {
		...
		default:
            ...
            if (VERBATIM > stmts->type && stmts->type > VERBATIM_BOTH_J) 
...
list_t* check_map_class_value(symbol_t* map, list_t* val)
{
	...
	while (val_sub != NULL) {
		...
		if (class_stmts->sp->modifiers == M_UNSIGNED 
            && class_stmts->sp->ptype->ident <= INT 
...
list_t* check_map_simple_value(symbol_t* map, list_t* val)
{
	if (map->ptype->ident != val->e1->type &&
        !(map->ptype->ident == FLOAT && val->e1->type == DOUBLE || map->ptype->ident == DOUBLE && val->e1->type == FLOAT))
...
void check_map_esc_class_value(symbol_t *map, list_t *val)
{
	...
	while (val_sub != NULL) {
	    ...
	    /* if char, check that the parse value is not larger than 8 */
	    if (val_sub->e2->type == CHAR && val->e3->left.ival > 8) 
...
void check_map_esc_simple_value(symbol_t *map, list_t *val)
{
	...
	/* if char, check that the parse value is not larger than 8 */
	if (val->e2->type == CHAR && val->e3->left.ival > 8) 
		yyerror("Parsed length for 'char' larger than 8 (%d)", val->e3->left.ival);
	/* bit length of integer parsable variable cannot be bigger than 32 */
	else if (val->e2->type == INT && val->e3->left.ival > 32)
		yyerror("Parsed length larger than 32 bits (%d)", val->e3->left.ival);
	/* bit length of float parable variable must be eaqual to 32 */
	else if (val->e2->type == FLOAT && val->e3->left.ival != 32)
		yyerror("Warning: length of float parsable variable must be 32");
	/* bit length of double parable variable must be eaqual to 64 */
	else if (val->e2->type == DOUBLE && val->e3->left.ival != 64) 
		yyerror("Warning: length of double parsable variable must be 64");
}
...
void check_bac_val(list_t *lp)
{
    ...
    /* sanity check */
    if (lp->e1 == NULL || lp->e1->op != INT_LITERAL)
    ...
    switch (param) {
    case BAC_PREC:
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 3 || np->left.ival > 32) {
            yyerror("Error: Expected an int value in [3,32] for the 'prec' param");
            exit(1);
        }
        break;
    case BAC_OOC:
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 0 || np->left.ival > 2) {
            yyerror("Error: Expected an int value in [0,2] for the 'ooc' param");
            exit(1);
        }
        break;
    case BAC_SOC:
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 0 || np->left.ival > 3) {
            yyerror("Error: Expected an int value in [0,3] for the 'soc' param");
            exit(1);
        }
        break;
    case BAC_TE:
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 0 || np->left.ival > 1) {
    ...
    case BAC_RENORM:
        ...
        if (lp->sub1->type == EXPR) {
            if (np == NULL || np->op != INT_LITERAL ||  np->left.uival < 0) {
            ...
            if (lp->sub1->sub1->next->next != NULL) {
                ...
                if (np == NULL || np->op != INT_LITERAL || np->left.ival != 1) {
    ...
    case BAC_BS:
        np = lp->sub1->sub1->e1;
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 2 || np->left.ival > 31) {
            yyerror("Error: Expected an int value in [2,31] for the first arg to the 'bs' param");
            exit(1);
        }
        np = lp->sub1->sub1->next->e1;
        if (np == NULL || np->op != INT_LITERAL || np->left.ival < 1 || np->left.ival > 8) { 
            yyerror("Error: Expected an int value in [1,8] for the second arg to the 'bs' param");
            exit(1);
        }
        if (lp->sub1->sub1->next->next != NULL) {
            np = lp->sub1->sub1->next->next->e1;
            if (np == NULL || np->op != INT_LITERAL || np->left.ival < 0 || np->left.ival > 7) {
    ...
    case BAC_INIT:
        if (lp->sub1->type == EXPR) {
            ...
            np = lp->sub1->sub1->next->e1;
            if (np == NULL || np->op != INT_LITERAL || np->left.ival < 0 || np->left.ival > 32) {
    ...
    case BAC_RTABLE:
        np = lp->sub1->sub1->e1;
        if (np == NULL || np->op != INT_LITERAL || np->left.ival <= 0) {
        ...
        np = lp->sub1->sub1->next->next->e1;
        if (np == NULL || np->op != STRING_LITERAL) {
