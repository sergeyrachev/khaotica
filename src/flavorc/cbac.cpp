...
static void c_init_bac_params(void)
{
    ...
    end_type = END_NONE;           /* no special action is needed */
...
static void c_bac_core(list_t *stmts)
{
    ...
    while (lp != NULL) {
        ...
        case flavor::Parser::token::BAC_OOC:
            ooc = (ooc_t)np->left.ival;
            break;
        case flavor::Parser::token::BAC_SOC:
            soc = (soc_t)np->left.ival;
            break;
        case flavor::Parser::token::BAC_TE:
            te = (te_t)np->left.ival;
            break;
