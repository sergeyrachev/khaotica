...
char *buildstr(char *str, int c)
{
    ...
    /* filled up, expand */
    if ((size % BUILDSTR_SIZE) == 0) str = regetmem(str, size+BUILDSTR_SIZE);
...
void **buildarray(void **p, void *n)
{
    ...
    if ((size % BUILDARRAY_SIZE) == 0) {
        /* filled up, expand */
        p = regetmem(p, (size+BUILDARRAY_SIZE)*sizeof(void *));
    }
    p[size-1] = n;
    p[size] = NULL;
    return p;
...
{
    if (id == BIT_LITERAL || id == INT_LITERAL ||  id == DOUBLE_LITERAL || id == STRING_LITERAL) return 1;
    else return 0;
}
...
node_t *n_bit2int(node_t *n)
{
    ...
    if (n->op != BIT_LITERAL) fatal("Internal error U%d: argument is not a bitstring literal", __LINE__);
    sp = n->left.bit;
    n->op = INT_LITERAL;
    n->type = INT;
...
int is_verbatim(int type) {
	if (VERBATIM <= type && type <= VERBATIM_BOTH_J) return 1;
	else return 0;
